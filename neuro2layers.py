...
class twolayernetwork:
	...
	def forwardprop(self, inputarr):
		...
		self.fermithresh(self.hiddenlayer[0:-1])
		...
		self.fermithresh(self.outputlayer[0,:-1])
	...
	def backprop(self):
		...
		for key in self.learningset.keys():
			...
			e=(outputlayer[0]-self.learningset[key])**2
			delta2=np.dot(D2, e)
			delta1=np.dot(np.dot(D1,self.W2), delta2)
